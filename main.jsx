...
import { Plus, Trash2, Calendar, Droplets, Leaf, Camera, ArrowLeft, TrendingUp, User } from 'lucide-react';

// --- Vercel公開用にエラーを防ぐための空設定 ---
...
const App = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => {
        ...
        console.log("Firestore Mode: Offline");
      });
      return () => unsubscribe();
    } catch (e) {
  ...
  const addLog = async (plantId) => {
    ...
    const logEntry = {
      ...
      note: newLogNote, 
      watered: newLogWatered, 
      fertilized: newLogFertilized, 
      image: newLogImage
    };
        ...
        {activeTab === 'detail' && selectedPlant && (
          <div className="space-y-6 animate-in fade-in duration-300">
            <div className="bg-emerald-50 p-4 rounded-3xl border border-emerald-100">
              <h3 className="font-bold text-emerald-800 mb-3 flex items-center gap-2 text-sm"><TrendingUp size={16}/> 成長比較 (2枚選択)</h3>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {selectedComparison.map((p, idx) => (
                    <div key={p.id} className="space-y-1">
                      ...
                      <p className="text-[10px] text-center font-bold text-stone-500">{idx === 0 ? '以前' : '現在'}</p>
                    </div>
                  ))}
                </div>
            ...
            <div className="flex justify-between items-end px-2">
              ...
              <button onClick={() => { if(window.confirm('削除しますか？')) deletePlant(selectedPlant.id) }} className="p-2 text-stone-300"><Trash2 size={18}/></button>
            </div>
            ...
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-stone-100 space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <label className="bg-stone-50 px-4 py-2 rounded-full text-xs font-bold cursor-pointer"><Camera size={14}/><input type="file" accept="image/*" className="hidden" onChange={e => handleImageChange(e, setNewLogImage)}/></label>
              </div>
              {newLogImage && <img src={newLogImage} className="w-20 h-20 object-cover rounded-xl mt-2 border-2 border-white shadow" />}
              <button onClick={() => addLog(selectedPlant.id)} className="w-full bg-stone-800 text-white py-3 rounded-xl font-bold shadow-lg">保存する</button>
            </div>
